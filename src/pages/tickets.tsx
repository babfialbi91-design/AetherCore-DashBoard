...
import { Ticket, Settings, List, MessageSquare, Plus, Trash2, Save, Eye, X } from "lucide-react";
...
type Ticket = {
  ...
  opener: { id: string; displayName: string; avatar: string | null };
  claimedBy: { id: string; displayName: string; avatar: string | null } | null;
};
...
export default function Tickets() {
  ...
  const { data: config, isLoading: loadingConfig } = useQuery({
    ...
    queryFn: () => apiCall<TicketConfig>("/api/bot/tickets/config"),
  });

  const { data: types, isLoading: loadingTypes } = useQuery({
    queryKey: ["ticket-types"],
    queryFn: () => apiCall<TicketType[]>("/api/bot/tickets/types"),
  });

  const { data: tickets, isLoading: loadingTickets } = useQuery({
    queryKey: ["tickets"],
    queryFn: () => apiCall<Ticket[]>("/api/bot/tickets"),
  });
  ...
  useEffect(() => {
    if (openMsgData?.message) setOpenMessage(openMsgData.message);
  }, [openMsgData]);
              ...
              {loadingConfig ? (
                ...
                <>
                  <div className="space-y-2">
                    <label className="text-sm font-medium">{t("ticketChannel")}</label>
                    <Select value={configState.channelId} onValueChange={(v) => setConfigState((s) => ({ ...s, channelId: v }))}>
                      <SelectTrigger className="bg-background/50"><SelectValue placeholder={channelsLoading ? t("loading") : t("selectChannel")} /></SelectTrigger>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">{t("staffRoles")}</label>
                    <div className="flex flex-wrap gap-2">
                      {roles?.map((role) => (
                        <Badge
                          key={role.id}
                          variant={configState.staffRoleIds.includes(role.id) ? "default" : "outline"}
                          className="cursor-pointer"
                          onClick={() => {
                            setConfigState((s) => ({
                              ...s,
                              staffRoleIds: s.staffRoleIds.includes(role.id)
                                ? s.staffRoleIds.filter((id) => id !== role.id)
                                : [...s.staffRoleIds, role.id],
                            }));
                          }}
                        >
                          {role.name}
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">{t("panelTitle")}</label>
                    <Input value={configState.panelTitle} onChange={(e) => setConfigState((s) => ({ ...s, panelTitle: e.target.value }))} className="bg-background/50" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium">{t("panelDescription")}</label>
                    <Textarea value={configState.panelDescription} onChange={(e) => setConfigState((s) => ({ ...s, panelDescription: e.target.value }))} className="bg-background/50 resize-none" />
                  </div>
                  <Button onClick={() => saveConfig.mutate(configState)} disabled={saveConfig.isPending || !configState.channelId || !configState.staffRoleIds.length}>
                    ...
                    {saveConfig.isPending ? t("loading") : t("saveTicketConfig")}
                  </Button>
                </>
              )}
            </CardContent>
              ...
              <CardContent>
                <Form {...typeForm}>
                  <form onSubmit={typeForm.handleSubmit((v) => addType.mutate(v))} className="flex gap-3 items-end">
                    <FormField control={typeForm.control} name="emoji" render={({ field }) => (
                      <FormItem className="w-24"><FormLabel>{t("emoji")}</FormLabel><FormControl><Input placeholder="🎫" {...field} className="bg-background/50" /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={typeForm.control} name="label" render={({ field }) => (
                      <FormItem className="flex-1"><FormLabel>{t("typeName")}</FormLabel><FormControl><Input placeholder={t("typeNamePlaceholder")} {...field} className="bg-background/50" /></FormControl><FormMessage /></FormItem>
                    )} />
                    <Button type="submit" disabled={addType.isPending} className="mb-0.5">
            ...
            <Card className="bg-card/50 border-white/5">
              <CardContent className="pt-6">
                {loadingTypes ? (
                  ...
                  <div className="space-y-2">
                    {types.map((type) => (
                      <div key={type.id} className="flex items-center justify-between p-3 bg-black/30 rounded-lg border border-white/5">
                        <div className="flex items-center gap-3">
                          <span className="text-xl">{type.emoji || "📁"}</span>
                          <div>
                            <p className="font-medium">{type.label}</p>
                            <p className="text-xs text-muted-foreground font-mono">{type.value}</p>
                          </div>
                        </div>
                        <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-destructive" onClick={() => deleteType.mutate(type.id)}>
                  ...
                  <p className="text-center text-muted-foreground py-8">{t("noTicketTypes")}</p>
                )}
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="bg-card/50 border-white/5">
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              {closedTickets.length > 0 && (
                <Button variant="destructive" size="sm" onClick={() => clearClosed.mutate()} disabled={clearClosed.isPending}>
            ...
            <CardContent>
              {loadingTickets ? (
                ...
                <div className="space-y-2">
                  {tickets.map((ticket) => (
                    <div key={ticket.id} className="flex items-center justify-between p-3 bg-black/30 rounded-lg border border-white/5">
                      <div className="flex items-center gap-3 min-w-0">
                        <Badge variant={ticket.status === "open" ? "default" : "secondary"} className="flex-shrink-0">
                          {ticket.status}
                        </Badge>
                        <div className="min-w-0">
                          <p className="font-medium truncate">{ticket.opener?.displayName || ticket.opener?.id}</p>
                          <p className="text-xs text-muted-foreground">{ticket.type} · {new Date(ticket.createdAt).toLocaleDateString()}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {ticket.hasTranscript && (
                          <Button variant="ghost" size="icon" className="text-muted-foreground" onClick={async () => {
                            const data = await apiCall<{ transcript: string }>(`/api/bot/tickets/${ticket.id}/transcript`);
                            if (data.transcript) {
                              const w = window.open("", "_blank");
                              w?.document.write(`<pre>${data.transcript}</pre>`);
                            }
                          }}>
                            <Eye className="w-4 h-4" />
                          </Button>
                        )}
                        <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-destructive" onClick={() => deleteTicket.mutate(ticket.id)}>
                          <Trash2 className="w-4 h-4" />
                        </Button>
                      </div>
                    </div>
